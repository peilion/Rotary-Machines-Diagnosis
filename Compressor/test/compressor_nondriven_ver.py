...
def chlorinecompressor_compressor_nondriven_end_vertical(xdata: ndarray, ydata: ndarray, pressure: ndarray,
                                                         fs: int, R: ndarray, th: ndarray):
    x = VibrationSignal(data=xdata, fs=fs, type=2)
    y = VibrationSignal(data=ydata, fs=fs, type=2)
    mp_instance = CP_Compressor_NonDriven_Vertical(x=x, y=y, r=R[1],
                                                   pressure=pressure,
                                                   ib_threshold=th[0:3],
                                                   wd_threshold=th[3:6],
                                                   al_threshold=th[6:9],
                                                   bl_threshold=th[9:12],
                                                   sg_threshold=th[12:15],
...
if __name__ == '__main__':
    ...
    res = chlorinecompressor_compressor_nondriven_end_vertical(xdata=1000 * data[:, 0],  # mm/s2
                                                               ydata=1000 * data[:, 1],
                                                               fs=25600,
